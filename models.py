...
class Model(object):
    ...
    def pool(self, input): #进行max_pool操作
        return tf.nn.max_pool(input, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    def get_wb(self, layers, i):#从预训练好的vgg模型中读取参数
        """                
    ...
    def get_random_img(self):#根据噪音和内容图片，生成一张随机图片
    ...
    def loadimg(self, path):#加载一张图片，将其转化为符合要求的格式        
        ...
        image = image - settings.IMAGE_MEAN_VALUE# 减去均值，使其数据分布接近0
        return image
